...
| Tournament delegated roles | Implemented locally for original organizers, one co-organizer, up to four tournament admins, and one press officer. A canonical server-side access route now resolves verified OAuth identity before legacy password fallback; role-list access is restricted to organizer/co-organizer/admin/implicit superadmin, co-organizers manage admins and press officers but not themselves, admins are read-only for roles, and press officers only see announcements. Delegated tournaments are merged into the authenticated managed-tournament menu. Password-only guest-admin access remains intentionally deferred | `066_tournament_roles.sql` plus `067_tournament_role_slot_indexes.sql`; migration application not verified | `npm run lint`, `npm test` (143), `npm run build`, server-import check, 12-function count, and `git diff --check` pending for this follow-up | Deploy/apply both migrations, then verify with real OAuth sessions for each role |
...
| APPG scoring and result entry | Implemented locally for organizer-managed and sandbox tournaments. Persisted `scoring_mode = 'appg'` now resolves to the internal APPG-120 profile, separating cooperative average-points aggregation from the English 120-minute ET3/ET2/PS1/RT0/OPW ruleset. Each classified outcome awards the same points to both participating teams; APPG averages divide total points by every counted completed match, including unclassified matches, with no minimum-match threshold. APPG outcomes remain explicit (`ET3`, `ET2`, `PS1`, `RT0`, `OPW`, or `needs_review`); real tournaments use bulk result editing and CSV import, while sandbox tournaments can random-fill a round or season. Newly added, manually linked, and live-refreshed finished APPG-120 matches use structured CHPP goal evidence for conservative auto-classification; organizer and CSV decisions remain authoritative. APPG history awards use completed-match eligibility and replace fixture-completion with most matches played; injury awards preserve counts while ranking by structured injury weeks when available | `060_add_appg_match_outcomes.sql`; migration file created, Supabase application not verified | `npm run lint`, `npm test` (129), `npm run build`, server import check, 12-function count, and `git diff --check` passed 2026-07-23 | Apply `060`, then test a disposable organizer-managed APPG-120 tournament with real Supabase data and verify cooperative points, all-match APPG averages, classification, unequal-match history awards, injury-week awards, add, link, refresh, and existing review rows |
| No pre-made schedule / Add HT match | Implemented locally. Admin Settings now uses a Generated schedule / No pre-made schedule dropdown. Generated keeps the existing round-robin flow; No pre-made shows suggested Hattrick matches first, can scan registered teams with bounded four-week `matchesArchive` requests for current/previous seasons, retains the regular upcoming and Last 50 paths, exposes collection/filter diagnostics, and keeps the manual single Match ID fetch/preview/add flow behind a text link. Adding suggested matches sequentially refreshes fixtures once, then normalizes manual rounds chronologically by calendar date, reusing or creating rounds safely and removing empty rounds without deleting matches. Organizers can rerun normalization independently; failures no longer mask successful imports. Clear results preserves fixtures and linked IDs while resetting APPG rows to valid unclassified values; clear fixtures remains the separate destructive path | `065_allow_manual_schedule_mode.sql` finalizes the earlier observed-schedule prototype by converting existing `observed` tournament rows to `manual` and setting the final schedule-mode constraint; match rows use existing fields from earlier fixture, APPG, structured-event, and observed-prototype migrations | `npm run build`, `npm test` (128), `npm run lint`, server import check, 12-function count, and `git diff --check` passed 2026-07-23 | Apply `065`, set a disposable tournament to No pre-made schedule, fetch current/previous-season matches between registered teams, inspect diagnostics, add older and newer matches, rerun normalization, and verify chronological round repair and clear-result/clear-fixture separation against live Supabase/CHPP data |
| Tournament seasons/history | Versioned v2 snapshots freeze participants, standings, matches, awards, records and an explicit season story. Closing a season no longer generates its report automatically; admins can generate or clear the report separately from Season Planner, and adding the next season requires a generated report. The public History tab uses a two-column archive with shared distinctions, all participants, final standings, records, a memorable match and immutable participant yearbook comments. Published reports have DB-backed per-manager tracking plus a local dismissal fallback, with a seven-day announcement window | `057` exists and is marked applied in-file; `058` adds yearbook comments and is pending application; report notice tracking reuses `050` dismissal records | `npm run build`, `npm test` (89), `npm run lint`, server import check, and `git diff --check` passed 2026-07-22. Chromium E2E spec remains unavailable because bundled Chromium and system Chrome abort with `SIGABRT` on this Intel Mac | Apply `058`, finish a real tournament without generating its report, then separately generate, clear, and regenerate the report against live Supabase |
| Next-season transition and fixture archive | Implemented locally. Finishing Season N can now preserve the exact rendered fixtures as a JSON archive, then either auto-start a roster-locked Season N+1 or open it for owner re-application. Historical fixtures can be browsed from the Fixtures tab without CHPP rechecks; the old roster is preserved as muted suggestions only for an open new season | `061` is applied and adds the fixture archive/re-application fields. `062` contains the pending atomic `start_tournament_season` RPC | `npm run build`, `npm test` (93), `npm run lint`, server import check, 12-function count, and `git diff --check` passed 2026-07-20 | Apply `062`, then exercise both Season 2 paths on a finished tournament with real Supabase data |
...
`npm run check:server-imports`, `npm run lint`, `npm test` (129), `npm run build`, the 12-function count, and `git diff --check` passed on 2026-07-23 for cooperative APPG-120 points and all-match APPG averages, bounded historical CHPP archive discovery, fetch diagnostics, idempotent chronological manual-round repair, APPG-safe result clearing, APPG history-award eligibility, injury-week award ranking, finished-season fixture immutability, and tournament refresh stabilization. The server import check scans `api/` and `shared/` and requires explicit runtime `.js` extensions for relative server imports. The counted function total remains `12` after consolidating presence, history, activity, Forge session, and Forge statistics behind `api/app.ts`, while CHPP testing remains consolidated behind `api/testing/index.ts`. Live Supabase migration/application, CHPP response verification, and Vercel verification remain outstanding.
