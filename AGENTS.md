...
Do not bump the version for every completed code change. Keep the current version during routine bug fixes, small UI adjustments, copy changes, tests, documentation, and follow-up cleanup. Bump the patch version only for a meaningful release-sized batch of changes, after checking with the user or when the user explicitly requests a release bump. Do not change the minor or major version unilaterally: when the accumulated work plausibly warrants a milestone such as `0.3.8` to `0.4.0`, suggest that release bump in the closeout instead. Keep `package.json` and `package-lock.json` versions synchronized when a bump is approved.
